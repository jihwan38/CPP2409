...
vector<vector<int>> LoadMap(string file_name, int mapX, int mapY);
// 메인  함수
int main() {
	// 0은 빈 공간, 1은 아이템, 2는 적, 3은 포션, 4는 목적지
	vector<vector<int>> map = LoadMap("map.txt", mapX, mapY);
	...
	while(1){
		try{
			...
			cin >> user_pro;
			if((user_pro != "Magician") && (user_pro != "Warrior")){
					throw user_pro;
			}
